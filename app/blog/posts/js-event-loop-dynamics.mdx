...
See this example, subscribing to a state and doing a console.log when it changes, what happens when you call the state change twice? In theory, running normally, 2 times. But we want automatic batching.
...
export function subscribe<T extends object>(
  ...
  const listener: Listener = (op) => {
    ...
    if (!promise) { // here it verifies if the promise exist, to skip the second+ updates
      promise = Promise.resolve().then(() => {// here it calls schedule the microtask
        promise = undefined
        if (isListenerActive) {
          callback(ops.splice(0))
        }
...
It doesn't use async await, so the return of prerender is a promise itself, when it’s sent as an argument to resolve, as a thenable object, the then will be “called” and the pre-rendered page will be sent.
